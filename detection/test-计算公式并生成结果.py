...
l = {"type": 3, "start": 4.0667, "end": 897,
     "curve": [{"Channel": Channel1, "W": 0.25, "V": -0.009},
               {"Channel": Channel2, "W": 0.25, "V": -0.006},
               {"Channel": Channel3, "W": 0.25, "V": -0.009}]}
l1 = {"type": 2, "start": 897, "end": 4652.4,
      "curve": [{"Channel": Channel1, "W": 0.25, "V": -0.009},
                {"Channel": Channel2, "W": 0.42, "V": -0.001},
                {"Channel": None, "W": 1, "V": 0}]}
l2 = {"type": 1, "start": 4652.4, "end": 5149.667,
      "curve": [{"Channel": Channel1, "W": 0.25, "V": -0.007},
                {"Channel": None, "W": 1, "V": 0},
                {"Channel": None, "W": 1, "V": 0}]}
layer = [l, l1, l2]
        ...
        for curve_item in curve_list:
            ...
            for lable_depth in label_depth_list:
                ...
                if label_start > layer_old_endDepth:
                    ...
                    print("函数 N数据 查 询 执行时间为：", run_time, "秒")
                    n_Depth_list = [doc for doc in n_Depth]
                    # 记录函数执行后的时间
                    end_time = time.time()
                    # 计算函数执行时间
                    run_time = end_time - start_time
                    print("函数 N数据 转换 执行时间为：", run_time, "秒", " 共：", len(n_Depth_list))
                ...
                # 计算函数执行时间
                label_run_time = label_end_time - label_start_time
                print("函数 Label数据 查 询 执行时间为：", label_run_time, "秒")
                ...
                # 计算函数执行时间
                label_run_time = label_end_time - label_start_time
                print("函数 Label数据 转换 执行时间为：", label_run_time, "秒", " 共：", len(Depth_list))
                ...
                # 计算函数执行时间
                for_run_time = for_end_time - for_start_time
                print("For 函数执行时间为：", for_run_time, "秒\n")
