...
# @Version :1.0
# @Descriptioon :
# @Link : https://github.com/RosebudAnwuri/TheArtandScienceofData
# @File :  test.py
# -*- coding: utf-8 -*-
"""
...
plt.show()
nrc_db = pd.read_table(
    "https://raw.githubusercontent.com/mhbashari/NRC-Persian-Lexicon/master/NRC-emotion-lexicon-wordlevel-alphabetized-v0.92.txt",
...
music_df.loc[:, features] = music_df.loc[:, ].apply(
    lambda row: pd.Series(get_spotify_features(row["Song"], row["artist_shortened"]), index=features), axis=1)

ind = np.linspace(0, 5100, num=5101 - 1)
for i in ind:
    music_df.loc[i, features] = pd.Series(
        get_spotify_features(music_df.loc[i, "Song"], music_df.loc[i, "artist_shortened"]), index=features)
    print
    i

music_df = pd.read_csv(
    'C:/Users/rose.anwuri/Documents/TheArtandScienceofData/Music Sentiment Analysis/data/music_data.csv')
